...
for line in hand:
    ...
    line = line.rstrip()
    # Find all occurrences that match the following pattern and capture the numerical part in a group:
    # Starts with X ('^X')
    # Followed by zero or more non-whitespace characters ('\S*')
    # Followed by a colon and a space (': ')
    # Followed by one or more digits or periods ('([0-9.]+)') - The parentheses create a capturing group.
    # Store the captured group(s) in the list x.
    x = re.findall(r'^X\S*: ([0-9.]+)', line)
    # Check if the length of the list x is greater than 0, meaning at least one match was found.
    if len(x) > 0:
        # If matches were found, print the list x to the console
        # which will contain the captured numerical values.
        print(x)
